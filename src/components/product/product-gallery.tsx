          ...
          <div className="hidden w-16 shrink-0 flex-col gap-2 sm:flex">
            {images.map((img, i) => (
              <button
                ...
                onClick={() => setIndex(i)}
                className={cn(
                  "relative aspect-square w-full shrink-0 overflow-hidden rounded-xl border-2 transition",
                  i === index ? "border-brand" : "border-ink/10 opacity-60 hover:opacity-100",
                )}
            ...
            {images.length > 1 && (
              <>
                <button
                  type="button"
                  onClick={prev}
                  className="absolute left-3 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-ink shadow-md transition hover:bg-white"
                ...
                <button
                  type="button"
                  onClick={next}
                  className="absolute right-3 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-ink shadow-md transition hover:bg-white"
              ...
              {images.map((img, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === index ? "w-4 bg-brand" : "bg-ink/15 hover:bg-ink/30",
                  )}
                />
              ))}
            </div>
